...
export function Lobby({ onJoin }: { onJoin: (id: string) => void }) {
  ...
  const fetchCrews = async () => {
    ...
    if (data) {
      const parsed = data
        .filter(row => !(row.data as any)?.deleted)
        .map(row => {
    ...
    <div className="crt crt-flicker" style={{ width: '100vw', height: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: '#0a0a0a', padding: '20px' }}>
      ...
      {showRegister ? (
        <div className="panel" style={{ width: '400px' }}>
          <h2 style={{ color: 'var(--color-phosphor)', borderBottom: '1px solid var(--color-phosphor)', paddingBottom: '10px' }}>REGISTER ORGANIZATION</h2>
          
          <label style={{ display: 'block', marginTop: '15px' }}>Company Name</label>
          <input 
            type="text" 
            value={newCrewName}
        ...
        <div className="panel" style={{ width: '600px', maxHeight: '50vh', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ overflowY: 'auto', flex: 1, paddingRight: '10px' }}>
            {loading ? <p>SYNCING WITH LOCAL X-BOAT RELAY TENDER...</p> : crews.length === 0 ? <p>NO ACTIVE ORGANIZATIONS FOUND.</p> : (
              crews.map(crew => (
